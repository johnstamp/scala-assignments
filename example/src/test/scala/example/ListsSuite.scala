...
class ListsSuite extends FunSuite {
  ...
  /**
   * Include negative numbers in the maximum list
   */
  test("max including negative numners") {
    assert(max(List(3, -7, 2)) === 3)
  }
  /**
   * Test for max including zero in list
   */
  test("max including zero") {
    assert(max(List(3, 0, 2)) === 3)
  }

  test("Test it throws a noSuchElement Exception") {
